...
**Canonical guest implementation:** C++/QuickJS only (`kernel/embed/guest_modules.js` → WASM). Host TypeScript is not a Node runtime. The old `packages/api/src/kernel/js-runtime.ts` fallback is frozen and scheduled for deletion (PLAN Phase 13b). Status below is **embed/WASM**, not JS.
